...
% 2015-08-18
% 14.5 m/s, thrust, 0 yaw: 00, 40 pitch: 24
% 12 m/s, thrust, 0 yaw: 00, 30  pitch: 33

% 2015-08-19
% 10 m/s, thrust, 0 yaw: 00, 22.9 pitch: 10
% 8 m/s, thrust, 0 yaw: 00, 16.8 pitch: 22
% 6 m/s, thrust, 0 yaw: 00, 11.5 pitch: 31
% 4 m/s, thrust, 0 yaw: 00, 6.9 pitch: 42
...
for i=1:length(file)
  ...
  control_roll(i,1:control_size(i)) = temp.ASCTEC_CTRL_COMMAND(:,2);
  control_pitch(i,1:control_size(i)) = temp.ASCTEC_CTRL_COMMAND(:,3);
  control_yaw(i,1:control_size(i)) = temp.ASCTEC_CTRL_COMMAND(:,4);
  ...
  rc_pitch(i,1:rc_size(i)) = temp.ASCTEC_RC_DATA(:,2);
  rc_roll(i,1:rc_size(i)) = temp.ASCTEC_RC_DATA(:,3);
  rc_thrust(i,1:rc_size(i)) = temp.ASCTEC_RC_DATA(:,4);  % 6
  rc_yaw(i,1:rc_size(i)) = temp.ASCTEC_RC_DATA(:,5);
  rc_enable(i,1:rc_size(i)) = temp.ASCTEC_RC_DATA(:,6);
...
%% Save
save('wind_speed_props_yaw0_pitch_mean','wind_speed_mean');
save('wspi_voltage1_props_yaw0_pitch_mean','wspi_voltage1_mean');
save('wspi_voltage2_props_yaw0_pitch_mean','wspi_voltage2_mean');
save('wind_speed_props_yaw0_pitch_std','wind_speed_std');
save('wspi_voltage1_props_yaw0_pitch_std','wspi_voltage1_std');
save('wspi_voltage2_props_yaw0_pitch_std','wspi_voltage2_std');
save('wind_speed_props_yaw0_pitch_stderr','wind_speed_stderr');
save('wspi_voltage1_props_yaw0_pitch_stderr','wspi_voltage1_stderr');
save('wspi_voltage2_props_yaw0_pitch_stderr','wspi_voltage2_stderr');
...
%   plot(control_time(trial,1:control_size(trial)),control_pitch(trial,1:control_size(trial)))
%   plot(control_time(trial,1:control_size(trial)),control_thrust(trial,1:control_size(trial)))
%   plot(rc_time(trial,1:rc_size(trial)),rc_enable(trial,1:rc_size(trial)))
%   plot(rc_time(trial,1:rc_size(trial)),rc_thrust(trial,1:rc_size(trial)))
%   plot(rc_time(trial,1:rc_size(trial)),rc_yaw(trial,1:rc_size(trial)))

  title('Wind Speed Vs. Time')
  xlabel('Time [s]')
  legend('Pitch','Thrust','RC','Location','SouthEast')
  hold off
end
